...
public class StringsUI extends JPanel {
	...
	private Strings account;

	private ServiceItem item;

	private final static Logger logger = Logger.getLogger(StringsUI.class
			.getName());

	public StringsUI(Object provider) {
		super();
		getAccessibleContext().setAccessibleName("Strings Tester");
		item = (ServiceItem) provider;

		if (item.service instanceof Strings) {
			account = (Strings) item.service;
			createUI();
		}
		...
		try {
			uiDesc = UIDescriptorFactory.getUIDescriptor(MainUI.ROLE,
					new UIComponentFactory(new URL[] { new URL(Sorcer
							.getWebsterUrl()
							+ "/accout-ui.jar") }, StringsUI.class.getName()));
		} catch (Exception ex) {
			logger.throwing(StringsUI.class.getName(), "getUIDescriptor", ex);
		}
		return uiDesc;
	}
}
